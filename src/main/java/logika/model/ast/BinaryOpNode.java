...
public class BinaryOpNode extends FormulaNode {
    ...
    public BinaryOpNode(final Token token, final FormulaNode left, final FormulaNode right) {
        super(token, Arrays.asList(left, right));
        this.operator = Objects.requireNonNull(token.getType(), "operator cannot be null");
        this.left = Objects.requireNonNull(left, "left cannot be null");
        this.right = Objects.requireNonNull(right, "right cannot be null");
    }

    public BinaryOpNode(final Token operator, final List<FormulaNode> subformulas) {
